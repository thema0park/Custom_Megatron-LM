...
def _communicate(tensor_send_next, tensor_send_prev, recv_prev, recv_next,
    ...
    if not override_scatter_gather_tensors_in_pipeline and \
        ...
        if recv_prev:
            # JK
            if tensor_recv_prev.dtype != torch.float:
                tensor_recv_prev = tensor_recv_prev.type(torch.float)
        ...
        if recv_next:
            # JK
            if tensor_recv_next.dtype != torch.float:
                tensor_recv_next = tensor_recv_next.type(torch.float)
...
def recv_backward(tensor_shape=None, dtype_=None, timers=None):
...
def send_backward(input_tensor_grad, tensor_shape=None, dtype_=None, timers=None):
