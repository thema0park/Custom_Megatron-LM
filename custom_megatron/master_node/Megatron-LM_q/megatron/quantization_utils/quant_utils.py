...
def linear_dequantize(input_q, scale, zero_point=None, inplace=False):
    """
...
class SymmetricQuantFunction(Function):
    ...
    def forward(ctx, x, k, specified_scale=None):
        ...
        if k == 8:
            new_quant_x = new_quant_x.type(torch.cuda.CharTensor)
        elif k == 16:
            new_quant_x = new_quant_x.type(torch.cuda.ShortTensor)
        elif k == 32:
            new_quant_x = new_quant_x.type(torch.cuda.IntTensor)
        else:
            raise ValueError("Unsupported Quantization Bit")
