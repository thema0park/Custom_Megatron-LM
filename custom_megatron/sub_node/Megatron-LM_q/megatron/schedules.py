...
from megatron.quantization_utils.quant_utils import linear_dequantize
...
def recv_forward(tensor_shapes, dtype=None, timers=None):
    #print("recv_forward")
    input_tensors = []
...
def recv_backward(tensor_shapes, dtype=None, timers=None):
    #print("recv_backward")
    output_tensor_grads = []
    for tensor_shape in tensor_shapes:
        if tensor_shape is None:
...
def send_forward(output_tensors, tensor_shapes, dtype=None, timers=None):
    #print("send forward")
    ...
    for (output_tensor, tensor_shape) in zip(output_tensors, tensor_shapes):
        if tensor_shape is None:
            continue
        p2p_communication.send_forward(output_tensor, tensor_shape, dtype_=dtype, timers=timers)


def send_backward(input_tensor_grads, tensor_shapes, dtype=None, timers=None):
    #print("send backward")
    ...
    for (input_tensor_grad, tensor_shape) in zip(input_tensor_grads, tensor_shapes):
        if tensor_shape is None:
            continue
        p2p_communication.send_backward(input_tensor_grad, tensor_shape, dtype_=dtype, timers=timers)
...
def forward_backward_pipelining_without_interleaving(forward_step_func,
    ...
    for i in range(num_warmup_microbatches):
        if args.quantize_forward:
            quantized_tensor = recv_forward(recv_tensor_shapes, dtype=dtype, timers=timers)
            if quantized_tensor[0] is not None:
                scaling_factor = recv_forward([(1, 1, 1)])
                timers('forward-dequantize').start()
                input_tensor = linear_dequantize(quantized_tensor, scaling_factor, inplace=False)
                timers('forward-dequantize').stop()
            ...
            # JK
            timers('forward-quantize').start()
            quantized_tensor, scaling_factor = model.module.quantization(output_tensor)
            timers('forward-quantize').stop()

            send_forward(quantized_tensor, send_tensor_shapes, dtype=dtype, timers=timers)
            send_forward(scaling_factor, [(1, 1, 1)])
        else:
        ...
        if not forward_only:
            ...
            if args.quantize_forward:
                deallocate_output_tensor(quantized_tensor[0])
    ...
    for i in range(num_microbatches_remaining):
        ...
        if forward_only:
            ...
            if last_iteration:
                input_tensor = None
                ## JK
                if args.quantize_backward:
                    timers('backward-quantize').start()
                    quantized_tensor_grad, scaling_factor = model.module.quantization_backward(input_tensor_grad)
                    timers('backward-quantize').stop()
            
                    send_backward(quantized_tensor_grad, recv_tensor_shapes, dtype=dtype, timers=timers)
                    send_backward(scaling_factor, [(1, 1, 1)])
                else:
    ...
    if not forward_only:
        for i in range(num_warmup_microbatches):
            ...
            ## JK
            if args.quantize_backward:
                quantized_output_grad = recv_backward(send_tensor_shapes, dtype=dtype, timers=timers)
                scaling_factor = recv_backward([(1, 1, 1)])

                timers('backward-dequantize').start()
                output_tensor_grad = linear_dequantize(quantized_output_grad, scaling_factor, inplace=False)
                timers('backward-dequantize').stop()
